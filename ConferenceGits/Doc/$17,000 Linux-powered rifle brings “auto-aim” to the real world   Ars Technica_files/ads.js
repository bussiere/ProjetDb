...
ars.pulldown_ad = function() {
  if (!ars.MOBILE) {
    var pbanner_start = new Date();
    try {
      var pbanner = cnp.ad.create(cnp.ad.refreshable, false);
      pbanner.addParameterString(ars.AD.kw);
      pbanner.addParameter({'sz':'970x418'});
      pbanner.load();
    } catch(e) {}
  }
};

ars.sidebar_ad = function(pos) {
  var pbanner_start = new Date();
  try {
    var pbanner = cnp.ad.create(cnp.ad.refreshable, false);
    pbanner.addParameterString('kw=' + pos + ';' + ars.AD.kw);
    pbanner.addParameter({'sz':'300x250'});
    pbanner.load();
  } catch(e) {}
};

ars.masthead_ad = function() {
  if (ars.MOBILE && window.screen.width <= 640) {
    document.write('<noscript><img src="http://ad.doubleclick.net/adj/ars.wap.dart/homepage;kw=homepage;!c=homepage;sz=320x50" width="0" height="0" style="width: 0px; height: 0px;" /></noscript><script type="text/javascript" src="http://ad.doubleclick.net/adj/ars.wap.dart/homepage;kw=homepage;!c=homepage;sz=320x50"></script>');
  }
  else {
    var pbanner_start = new Date();
    try {
      var pbanner = cnp.ad.create(cnp.ad.refreshable, false);
      pbanner.addParameterString('kw=top;' + ars.AD.kw);
...
ars.hostcheck = function() {
  if (window.location.host.match(/917wy/)) {
    window.location.host = "arstechnica.com";
...
try {
  ars.hostcheck();
  var site = window.navigator.userAgent.match(/ipad/i) ? "ars.dart.ipad" : "ars.dart";
  cnp.ad.dart.setSite(site);
  cnp.ad.dart.setZone(ars.AD.zone);
  cnp.ad.emptyFrameSrc = ars.AD.empty;
  cnp.ad.loaderFrameSrc = ars.AD.loader;
} catch(e) {};
